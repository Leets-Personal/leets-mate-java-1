...
public class LeetsMateApplication {
    ...
    public boolean checkHasNoEnglish(String member) {
         ...
         */
        String test = "";
        System.arraycopy(member, 0, test, 0, member.length());

        if (!Pattern.matches(REGEXP_KR, test.replaceAll(",", "").replaceAll(" ", ""))) {
            System.out.println(ErrorResponse.of(ErrorCode.INPUT_FORMAT_ERROR));
            /**
             * 여기서 그냥 프로그램을 종료시키는지 에러 메세지만 반환하는 것인지 명시되어 있지 않아서
             * UX를 위해 제대로 된 입력을 받을 때 까지 다시 입력받는 형태로 구현하였습니다.
             */
            return true;
        }
        return false;
    ...
    public boolean checkDataValidity(int memberSize, int maximumGroupSize) {
        boolean flag = (memberSize < maximumGroupSize) ? true : false;
        if (flag) System.out.println(ErrorResponse.of(ErrorCode.GROUP_SIZE_ERROR));
        return flag;
    }
